...
MATRIX_CODE_CHARS ="աբգդեզէըթժիլխծկհձղճմյնշոչպջռսվտրցուփքևօֆԱԲԳԴԵԶԷԸԹԺԻԼԽԾԿՀՁՂՃՄՅՆՇՈՉՊՋՌՍՎՏՐՑՒՓՔՕ՛՜՝։֊֏֍"
...
class FallingChar(object):
    ...
    def reset(self, width, MIN_SPEED, MAX_SPEED):
        ...
        # offset makes sure that chars with same
        # speed don't move all in same frame
...
def main():
    ...
    if USE_COLORS:
        ...
        curses.init_pair(COLOR_CHAR_NORMAL, curses.COLOR_GREEN,
                         curses.COLOR_BLACK)
        curses.init_pair(COLOR_CHAR_HIGHLIGHT, curses.COLOR_WHITE,
                         curses.COLOR_GREEN)
        curses.init_pair(COLOR_WINDOW, curses.COLOR_GREEN,
                         curses.COLOR_GREEN)
    ...
    for i in range(DROPPING_CHARS):
        line = FallingChar(width, MIN_SPEED, MAX_SPEED)
        line.y = randint(0, height - 2)
        lines.append(line)
